define(['jquery'], function($) {
    function init(container) {
        ...
        if (location.href.indexOf("#") > 0) {
            var target = location.href.substring(location.href.indexOf("#"), location.href.length);
            if ($(target).hasClass('active')) {
                $(target).toggleClass('active').find('.services-list').slideUp();
            } else {
                $(target).toggleClass('active').find('.services-list').slideDown();
            }
        }

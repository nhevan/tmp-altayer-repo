define(['jquery', 'underscore', 'backbone', 'base/modules/carousel-slider', 'base/modules/animate', 'videojs', 'base/views/video_youtube2', 'base/modules/window-onload', 'base/modules/eventDispatcher'], function ($, _, backbone, carouselSlider, animate, videojs, YoutubeApi, windowOnload, eventDispatcher) {
    ...
    function backboneInit() {
        return Backbone.View.extend({
            ...
            events: {
                'click .header-slider__arrow': 'scrollDown'
            },
            ...
            animateScrollDown: function () {
                var arrowDown = this.$el.find('.header-slider__arrow');
                if (arrowDown && arrowDown.length > 0) {
                    var animateArrow = function (translateY, callback) {
                        animate(arrowDown, {
                    ...
                    var delayAnimate = setTimeout(function () {
                        ...
                        var playAnimate = function () {
                            animateArrow(-25, function () {
                                animateArrow(0, function () {
                                    var replayAnimation = setTimeout(function () {
                                        clearTimeout(replayAnimation);
                                        playAnimate();
                                    }, 1000);
                                });
                            });
            ...
            scrollDown: function () {
                ...
                animate($('body'), 'scroll', { offset: height + top, duration: 500, easing: "easeInOutQuad" });
            },
            ...
            initSlider: function () {
                ...
                var option = {
                    ...
                    onResized: _.bind(this.carouselResized, this)
                };
                var sliderItems = this.$el.find('.header-slider__item');
                this.numberItems = sliderItems.length;
                ...
                var i = 1;
                this.$el.children().find('.owl-dot').each(function () {
                    $(this).text(i);
                    i++;
                });
            },
            ...
            carouselChange: function (event) {
                var isYoutube = $(event.target).find('.owl-item').eq(event.item.index).find('.header-slider__item .vehicle-detail-video-container');
            ...
            hidePagingForMobile: function () {
                if (this.$el.find('.header-slider__item__text').css('position') != 'absolute') {
                    this.$el.find('.owl-dots').css({ 'display': 'none', 'visibility': 'hidden' });
                }
            },
            ...
            getFirstActiveSlide: function(numberItems, isLoop) {
                if(!isLoop)
                    return 0;
            ...
            initVideo: function () {
                ...
                var isInit = false;
                var indexActive = this.getFirstActiveSlide(this.numberItems, this.isLoop);

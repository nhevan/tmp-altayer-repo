define(['jquery', 'underscore', 'backbone', 'base/views/map_infowindow', 'base/views/map_marker', 'lib/modernizr'], function ($, _, backbone, mapInfoWindow, mapMarker) {
    ...
    function backboneInit() {
        return Backbone.View.extend({
            ...
            imageShowroom: "/assets/img/marker_showroom.png",
            imageServiceCenter: "/assets/img/marker_service.png",
            imageBodyshop: "/assets/img/marker_bodyshop.png",
            ...
            initMap: function () {
                ...
                this.map.setOptions({ minZoom: 5, maxZoom: 12 });
            ...
            showInfoWindow: function (i) {
                ...
                var worktime = "";
                var phone = "";
                if (this.locations[i].workTime) {
                    worktime = "<div class='infowindow__content__watch'>" + this.locations[i].workTime + "</div>";
                }
                if (this.locations[i].phone) {
                    phone = "<a class='infowindow__content__phone' href='tel:" + this.locations[i].phone + "'>" + this.locations[i].phone + "</a>";
                }

                var href = "http://maps.google.com/maps?z=12&t=m&q=loc:" + this.locations[i].lat + "+" + this.locations[i].lng;
                if (this.locations[i].GoogleMapUrl && this.locations[i].GoogleMapUrl.length > 0) {
                    href = this.locations[i].GoogleMapUrl;
                }

                var direction = "<a href='" + href + "' class='infowindow__content__direction' target='_blank'>click here for directions</a>";
                var directionAF = "<a href='" + href + "' class='infowindow__content__direction-af' target='_blank'>انقر هنا للاطلاع على الاتجاهات</a>";
